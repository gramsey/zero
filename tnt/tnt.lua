...
local function add_drop(drops, item)
	...
	local blast_loss = minetest.get_item_group(name, "blast_loss")
	
	-- 0 = always dropped, 1 = never, 2 = 1/2 chance, 3 = 1/3 ... 
	if blast_loss ~= 0 and math.random(1, blast_loss) == 1 then
		return
	end
...
minetest.register_craft({
	...
	type = "shapeless",
	recipe = {"coal:lump", "gravel:gravel"}
})
...
if enable_tnt then
	...
	minetest.register_abm({
		...
		neighbors = {"fire:basic_flame", "lava:lava_source", "lava:lava_flowing"},
		interval = 4,
		chance = 1,
