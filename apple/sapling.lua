...
function apple.grow_apple_sapling(pos) 
    ...
    local tree_schema = minetest.get_modpath("apple") .. "/schematics/apple_tree_from_sapling.mts"
    minetest.place_schematic({x=pos.x-3, y=pos.y-1, z=pos.z-3}, tree_schema, "random", nil, false)
end

minetest.register_node("apple:sapling", {
    ...
    walkable = false,
    on_timer = apple.grow_apple_sapling,
    selection_box = {
        type = "fixed",
...
minetest.register_lbm({
    name = "apple:convert_saplings_to_apple_tree",
    nodenames = {"apple:sapling"},
    action = function(pos)
        minetest.get_node_timer(pos):start(math.random(300, 1500))
    end
})
