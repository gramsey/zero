...
-- export sounds so they can be easily accessed by other mods
-- without directly referencing files
fire.sound = {
	extinguish = "fire_extinguish_flame",
	roaring = "fire",
	gentle = "fire_small",
	strike = "fire_flint_and_steel",
}
...
-- Flint item
minetest.register_craftitem("fire:flint", {
    description = S("Flint"),
    inventory_image = "fire_flint.png"
})
...
if flame_sound then
	...
	function fire.update_player_sound(player)
		...
		if flames > 0 then
			...
			-- Play sound
			local handle = minetest.sound_play(fire.sound.roaring, {
				pos = fposmid,
